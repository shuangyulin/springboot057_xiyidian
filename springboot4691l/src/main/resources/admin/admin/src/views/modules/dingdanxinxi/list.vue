<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="家庭地址">
		     <template slot-scope="scope">
                  ...
                  <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="zhifupingzheng"
                    header-align="center"
                    width="200"
                    label="支付凭证">
                    <template slot-scope="scope">
                      <div v-if="scope.row.zhifupingzheng">
                        <img :src="scope.row.zhifupingzheng.split(',')[0]" width="100" height="100">
                      </div>
                      <div v-else>无图片</div>
                    </template>
                  </el-table-column>
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','上传进度') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="dingdanjinduCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'上传进度':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','上传进度') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="dingdanjinduCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'上传进度':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','上传进度') && contents.tableBtnIcon == 0" type="success" size="mini" @click="dingdanjinduCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'上传进度':'' }}</el-button>
    ...
    <dingdanjindu-cross-add-or-update v-if="dingdanjinduCrossAddOrUpdateFlag" :parent="this" ref="dingdanjinduCrossaddOrUpdate"></dingdanjindu-cross-add-or-update>
...
import dingdanjinduCrossAddOrUpdate from "../dingdanjindu/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      dingdanjinduCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    dingdanjinduCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.dingdanjinduCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$nextTick(() => {
      this.$refs.dingdanjinduCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          yifuleixing: row.yifuleixing,
          yifushuliang: row.yifushuliang,
          dianjiazhanghao: row.dianjiazhanghao,
          dianjiaxingming: row.dianjiaxingming,
          gukezhanghao: row.gukezhanghao,
          gukexingming: row.gukexingming,
          jiatingdizhi: row.jiatingdizhi,
          zhifupingzheng: row.zhifupingzheng,
          beizhu: row.beizhu,
          sfsh: row.sfsh,
          shhf: row.shhf,
          id: row.id
